...
**Status 2026-07-13:** 2.2 fully done — the `quiz` branch merge added a per-assessment builder page (`/faculty/assessments/[id]`) with question types/points, weighted scoring criteria (migrations 016–018, criteria breakdown shown in student results), and section targeting; on top of that, AI-assisted question generation (`POST .../questions/generate`, Gemini→OpenRouter via shared `app/lib/ai/generate.ts`, drafts reviewed before save) and CSV import (client-side parse to drafts + downloadable template) landed on the builder page. A faculty Audit Trail page also exists (`/faculty/audit`). Remaining in Phase 2: only the FCM push half of 2.9. ~~⚠️ Compliance watch: "clear audit trail" DELETE~~ **Resolved 2026-07-13** — the DELETE handler, UI, and `clearAuditTrail()` helper were removed (F7 append-only restored) and `/api/faculty/audit` GET/POST now require a faculty/admin session (they previously had no auth at all). The stray hand-made `faculty_audit_logs` table (never in any migration) was consolidated into the canonical append-only `audit_logs` via migration 019 (data folded in, table dropped — validated on a local PG 18 chain run); `/api/faculty/audit` now reads/writes `audit_logs` with the actor taken from the session, faculty scoped to their own entries and admins seeing all.

**Status 2026-07-13 (admin side / F12, DFD 1.0):** all admin mock pages de-mocked. New admin APIs (audit-logged): `GET/POST /api/admin/users` + `PATCH/DELETE /api/admin/users/[id]` (provisioning with temp password + `force_password_change`; student invitations emailed, other roles shown the password once), `GET /api/admin/students` (+ `/[id]` with attempt history), `GET /api/admin/faculty` and `PUT /api/admin/faculty/[id]/students` (roster replacement on `faculty_students`). Dashboard (`/admin`) rewritten as a dynamic server component on live OLTP data (at-risk from `performance_predictions`, scores from `assessment_attempts`, activity from `audit_logs`, room occupancy from `room_assignments`); Users / Faculty / Faculty-Assignment / Student-Management pages wired to the new APIs; `admin/students/[id]` now uses the admin detail route + real predictions/competency scores/scenario history (Phase 3.8 known debt cleared). 2.8 now fully done: `/admin/audit` (nav "Activity Log", also linked from Reports and the dashboard) reads `GET /api/admin/audit` — full cross-role trail with action search, actor-role/entity-type/date-range filters, offset pagination, and per-page CSV export; verified live against the dev server with a minted admin session. The Reports page's four cards now link to their real destinations.
...
**Status 2026-07-14: 5.2 done** — all 15 screens off mocks; `lib/mocks.ts` deleted. Dashboard/Tasks/task-detail run on `scenario_assignments` (task detail is now a scenario runner: checklist + timer + `POST …/complete`, mirroring the web runner's scoring). Quizzes list groups assigned/available/completed from `/api/student/assessments`; quiz-taking uses the real attempt lifecycle (start → answer → submit-all → server-graded review with explanations; countdown + auto-submit for time-limited assessments — correct answers never reach the client before submit). EHR list/detail/TPR/IVF use `patients` + `/api/student/ehr` (IVF rewritten from the mock intake-output ledger to the real infusion model with ongoing/completed/discontinued lifecycle); TPR/IVF/vitals writes fall back to the 5.3 outbox offline. Vitals detail gained a working encode form (the mock button was inert) surfacing server/local anomaly flags (5.5). Notifications, progress, recommendations, profile, and the tab-bar unread badge all read their real endpoints. Added `hooks/useApiData.ts` (load + pull-to-refresh + stale-cache state). Verified end-to-end against live Supabase with a test student: scenario assign→complete (score 88), attempt flow (10 questions, graded 2/10, explanations), vitals anomaly (HR 135 flagged critical), TPR + IVF lifecycle, progress reflects attempts. Note: `/api/student/assessments` filters by `users.section` vs `target_sections` — students with no section only see untargeted assessments.

**Status 2026-07-15: patient↔scenario link + student patient scoping** — migration 020 adds `scenarios.patient_id` (FK → `patients`, kept by the faculty create/AI-save/suggest flows and a new patient dropdown in the manual create modal). Students no longer see the whole roster: `/api/patients` returns only patients linked to their assigned scenarios (`app/lib/assigned-patients.ts`), and student vitals/EHR writes 403 on any other patient. `/api/student/scenarios` and the scenario detail expose `patient_id`; the mobile runner shows an "Assigned Patient" card deep-linking to the patient chart and vitals encode. ⚠️ Migration 020 must be run in the Supabase SQL editor before the new queries work — pre-migration, patient/scenario routes 500 on the missing column. Existing scenarios have `patient_id = null` — backfill by hand (PATCH `/api/faculty/scenarios/[id]` accepts `patient_id`; there is no scenario-edit UI yet).
