...
function isPatientCritical(patient: FacultyPatient): boolean {
  const v = patient.vital_signs;
  if (!v) return false;
  const hr = v.heart_rate ?? 0;
  const temp = v.temperature ?? 0;
  const rr = v.respiratory_rate ?? 0;
  const spo2 = v.oxygen_saturation ?? 100;
  return hr > 100 || hr < 50 || temp > 37.5 || temp < 35.5 || rr > 20 || spo2 < 95;
}
...
export default function FacultyEhrClient() {
  ...
  const roomOptions = useMemo(() => {
    ...
    for (const p of patients) {
      if (p.room_number) set.add(p.room_number);
    }
  ...
  const filteredPatients = useMemo(() => {
    ...
    if (ageMin) {
      ...
      if (!isNaN(min)) list = list.filter((p) => p.age >= min);
    }
    if (ageMax) {
      const max = Number(ageMax);
      if (!isNaN(max)) list = list.filter((p) => p.age <= max);
    }

    if (criticalityFilter === "critical") {
      list = list.filter(isPatientCritical);
    } else if (criticalityFilter === "normal") {
      list = list.filter((p) => !isPatientCritical(p));
    }

    if (roomFilter) {
      list = list.filter((p) => p.room_number === roomFilter);
    }

    return list;
  }, [patients, search, genderFilter, ageMin, ageMax, criticalityFilter, roomFilter]);
  ...
  const activeFilterCount = useMemo(() => {
    ...
    if (roomFilter) count++;
    return count;
  }, [search, genderFilter, ageMin, ageMax, criticalityFilter, roomFilter]);
          ...
          <div className="bg-surface rounded-xl border border-hairline shadow-[0_1px_3px_0_rgba(0,0,0,0.04)] p-4">
            <div className="flex flex-wrap gap-3">
              <div>
                ...
                <select value={genderFilter} onChange={(e) => setGenderFilter(e.target.value)} className={inputClass}>
                  {GENDERS.map((g) => (
              ...
              <div>
                <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Age Min</label>
                <input type="number" min={0} max={150} value={ageMin} onChange={(e) => setAgeMin(e.target.value)} placeholder="Min" className={inputClass} />
              </div>
              <div>
                <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Age Max</label>
                <input type="number" min={0} max={150} value={ageMax} onChange={(e) => setAgeMax(e.target.value)} placeholder="Max" className={inputClass} />
              </div>
              <div>
                <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Criticality</label>
                <select value={criticalityFilter} onChange={(e) => setCriticalityFilter(e.target.value)} className={inputClass}>
                  {CRITICALITY.map((c) => (
              ...
              <div>
                <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Room</label>
        ...
        {loading ? (
          ...
          <>
            <div className="flex items-center gap-2 text-sm text-gray-500">
              ...
              <span>{filteredPatients.length} patient{filteredPatients.length !== 1 ? "s" : ""} found</span>
            </div>
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-sm text-gray-500">
                        <FontAwesomeIcon icon={faCake} className="w-3 h-3 mr-0.5" />
                        {patient.age}yo
                        <FontAwesomeIcon icon={faPerson} className="w-3 h-3 ml-2 mr-0.5" />
                  ...
                  <div className="space-y-1.5 text-sm">
                    <div className="flex items-center gap-2 text-gray-600">
                      ...
                      {isPatientCritical(patient) && (
                        <span className="ml-auto px-2 py-0.5 text-[10px] font-bold uppercase bg-rose-100 text-rose-700 rounded">
                          Critical
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-2 text-gray-600">
                      <FontAwesomeIcon icon={faNotesMedical} className="w-3 h-3 text-gray-400" />
        ...
        <div>
          ...
          <div className="flex items-center gap-3 text-sm text-gray-500 mt-0.5">
            <span className="flex items-center gap-1">
              <FontAwesomeIcon icon={faCake} className="w-3 h-3" />
              {selectedPatient.age}yo {selectedPatient.gender === "M" ? "Male" : "Female"}
            </span>
            <span className="flex items-center gap-1">
              <FontAwesomeIcon icon={faHashtag} className="w-3 h-3" />
              {selectedPatient.room_number || "No room"}
            </span>
            {isPatientCritical(selectedPatient) && (
              <span className="px-2 py-0.5 text-[10px] font-bold uppercase bg-rose-100 text-rose-700 rounded">
                Critical
              </span>
            )}
            <span className="px-2 py-0.5 bg-gray-100 text-xs text-gray-600 rounded truncate max-w-[200px]">
